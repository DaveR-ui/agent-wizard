...
function tabButton(fixture: { nativeElement: HTMLElement }, label: string): HTMLButtonElement {
  const tabs = Array.from(
    fixture.nativeElement.querySelectorAll<HTMLButtonElement>('.level-tab'),
  );
...
describe('RulesPanel', () => {
  ...
  it('should switch to the Grupos level and select families', () => {
    ...
    expect(pills).toEqual(['coders', 'guardians', 'exploration', 'quality', 'writers', 'coordination']);
    ...
    expect(compiled.querySelector('.family-members')?.textContent).toContain('coder-angular, coder-go');
  ...
  it('should render the 6 agent-specific rule blocks', () => {
    ...
    expect(heads).toEqual(['delivery', 'orchestrator', 'interpreter', 'explorer', 'reviewer', 'analista']);
  ...
  it('should filter within the Grupos level', () => {
    ...
    // Only coders-01 mentions slices in its description.
    expect(titles).toEqual(['Complexity review and slice routing']);
  });
